...
vocab_path = "chinese_roberta_wwm_ext_L-12_H-768_A-12_pytorch/vocab.txt"  # roberta模型字典的位置
word2idx, keep_tokens = load_chinese_base_vocab(vocab_path, simplfied=True)
model_name = "roberta"  # 选择模型名字
model_path = "chinese_roberta_wwm_ext_L-12_H-768_A-12_pytorch/pytorch_bert_model.bin"  # 模型位置
...
class BertDataset(Dataset):
    ...
    def __init__(self) :
        ...
        ## 拿到所有文件名字
        self.txts = glob.glob('THUCNews/*/*.txt')
    ...
    def iteration(self, epoch, dataloader, train=True):
        ...
        for token_ids, token_type_ids, target_ids in tqdm(dataloader, position=0, leave=True):
            ...
            if step % 1000 == 0:
                ...
                self.bert_model.train()
            if step % 8000 == 0:
                self.save("bert_auto_title_model_epoch_{}.bin".format(epoch))
